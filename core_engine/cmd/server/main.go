...
func triggerRazorpaySync(db *mongo.Database) error {
	...
	for collName, url := range endpoints {
		...
		req.Header.Set("User-Agent", "RevenueIQ-CoreEngine/1.0")
...
func main() {
	...
	if mongoErr != nil {
		...
		log.Printf("Successfully connected core_engine directly to MongoDB database 'revenueiq_db'")
		log.Printf("Executing initial sync from live Razorpay account natively in Go...")
		_ = triggerRazorpaySync(db)
	}

	log.Printf("Connecting core_engine to MongoService gRPC at %s...", mongoServiceAddr)
	...
	if err != nil {
		...
		log.Printf("Successfully connected core_engine to MongoService gRPC client!")
	}
	...
	r.GET("/healthz", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"status": status, "service": "core-engine", "mongo": mongoStatus})
	})
	...
	api := r.Group("/api/v1")
	{
		api.POST("/sync", func(c *gin.Context) {
			...
			recordAuditLog(db, "core-engine", "RAZORPAY_ACCOUNT_SYNC", "ACCOUNT", "razorpay_merchant", "razorpay_sync_worker", "Synced live Razorpay payments, orders, settlements into MongoDB", []string{"HMAC_VERIFICATION"}, "SUCCESS")
	...
	log.Printf("Starting Consolidated RevenueIQ Core Go Engine on :%s", httpPort)
	if err := r.Run(":" + httpPort); err != nil {
		log.Fatalf("Core Engine server error: %v", err)
	}
}
