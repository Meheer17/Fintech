        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
          ...
          <tbody>
            {filtered.map((log) => {
              ...
              const guardrails = Array.isArray(log.guardrails_checked) ? log.guardrails_checked : ['HMAC_VERIFICATION'];
                      ...
                      <td colSpan={6} style={{ padding: '16px 20px' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '12px' }}>
                          <div>
                            ...
                            <span style={{ color: '#1a1f2c', fontWeight: 500 }}>"{log.reasoning}"</span>
                          </div>
                          <div>
                            <span style={{ fontWeight: 600, color: '#8c98a9' }}>Actor / Agent: </span>
                            <span className="mono" style={{ color: '#4263eb' }}>{log.actor}</span>
                          </div>
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '4px' }}>
                            <ShieldCheck size={14} color="#2b8a3e" />
                            <span style={{ fontWeight: 600, color: '#8c98a9' }}>Guardrails Verified: </span>
                            {guardrails.map((g) => (
                              <span key={g} style={{ padding: '2px 6px', borderRadius: '4px', backgroundColor: '#ffffff', border: '1px solid #dee2e6', fontSize: '11px' }}>
                                {g}
                              </span>
                            ))}
                          </div>
